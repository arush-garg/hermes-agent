...
def restore_primary_runtime(agent) -> bool:
    ...
    # ── /keep prefix: skip primary restore for this turn ── (fork feature)
    ...
    try:
        ...
        # Re-register agent in monitor tool's registry so periodic monitors survive
        # the restore_primary path (which recreates the client without re-running
        # agent_init). Fixes monitors going silent after restore (fork feature).
        try:
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    if agent.provider == "claude-cli" or str(client_kwargs.get("base_url", "")).startswith("claude-cli://"):
        ...
        # ClaudeCLIClient spawns the first-party `claude` binary and exposes Hermes'
        # full tool surface to it via an in-process MCP bridge, so it needs a handle
        # to the live agent (fork feature). Strip OpenAI-only kwargs it doesn't accept.
        safe_kwargs = {
            k: v for k, v in client_kwargs.items()
...
def apply_pending_yolo_action(agent) -> None:
    """Apply a queued session-scoped ``/yolo`` toggle at a tool-batch boundary."""
    ...
    try:
        ...
        _ra().logger.info(
            "/yolo steer applied for session %s: %s", session_key, "enabled" if enable else "disabled"
        )
    except Exception as exc:
...
__all__ = [
    ...
    "extract_api_error_context", "apply_pending_steer_to_tool_results", "apply_pending_yolo_action", "_iter_pool_sockets",
