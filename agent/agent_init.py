...
_CONTROL_STATE: Dict[str, Any] = {
    ...
    # Pending /yolo toggle — (session_key, enable: bool) queued to fire after the next tool call,
    # same timing contract as _pending_steer.
    "_pending_yolo_action": None,
    # Messages that arrived while context compression was running: injected into the compressed
    # transcript once it completes (they become real user turns). Guarded by the lock.
    "_pending_compression_queue": list,
    "_pending_compression_lock": threading.Lock,
...
_TURN_STATE: Dict[str, Any] = {
    ...
    # Structured metadata from the latest FAILED API attempt in this turn; reset before every
    # turn so a cached agent cannot leak a previous turn's failure into a later decision.
    "_last_api_error_context": None,
}
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # Register agent in monitor tool's agent registry so periodic monitor ticks can inject
    # steer messages into this session.
    try:
...
def _apply_agent_section(agent, _agent_cfg):
    ...
    # Samples own RSS on a daemon thread and (a) logs + dumps all thread stacks when a warn
    # threshold is crossed, (b) exits the process with a distinctive code before the kernel
    # OOM killer has to fire when a hard threshold is configured. One thread per process —
    # idempotent across the per-request AIAgent instances that serve/gateway create.
    try:
        ...
        start_memory_watchdog(
            _resolve_memory_watchdog_settings(_agent_section.get("memory_watchdog"))
        )
    except Exception:
...
def _build_context_engine(agent, _agent_cfg, cs, _custom_providers, _effective_context_length, session_db):
    ...
    # Encrypted Codex reasoning replay window (compression.reasoning_replay_keep_last).
    ...
    if _cc is not None:
        _cc.reasoning_replay_keep_last = clamp_reasoning_replay_keep_last(
            _parse_config_int(
                _cfg_dict(_agent_cfg, "compression").get(
...
_USAGE_STATE: Dict[str, Any] = {
    ...
    "session_api_calls": 0,
    # Counts provider calls that supplied usable token usage. Separate from session_api_calls
    # because some runtimes count completed calls even when the provider omits usage metadata.
...
def init_agent(
    ...
    checkpoint_max_file_size_mb: int = 10, checkpoint_interval: int = 10, pass_session_id: bool = False,
