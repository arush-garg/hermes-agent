...
def _replay_reasoning_items(
    ...
    keep_reasoning_at: Optional[frozenset] = None,
) -> List[Dict[str, Any]]:
    ...
    for ri in _as_list(msg.get("codex_reasoning_items")):
        ...
        if (item_id and item_id in seen_item_ids) or (ri.get("type") == "compaction" and not native_compaction_eligible):
            continue
        # Freshness bound: drop ordinary encrypted reasoning outside the keep
        # window. Native compaction checkpoints are not a stale interpretation —
        # leave them to the gate above.
        if (
            keep_reasoning_at is not None
            and ri.get("type") != "compaction"
...
def _chat_messages_to_responses_input(
    ...
    ``reasoning_replay_keep_last`` bounds *which* assistant turns still replay encrypted reasoning between
    compactions. Reasoning on turns at or before the latest compaction summary is always dropped; among the
    remaining reasoning-bearing assistant turns only the last N are replayed (default 8; ``0`` = no extra turn
    cap). Persisted history is not mutated — this is request-assembly only.
    """
    ...
    for msg_index, msg in enumerate(messages):
        ...
        reasoning_items = [] if not replay_encrypted_reasoning else _replay_reasoning_items(
            ...
            msg_index=msg_index, keep_reasoning_at=keep_reasoning_at,
        )
