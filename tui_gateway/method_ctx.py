...
def bind_module(module_globals: dict, server, *, skip=()) -> None:
    ...
    for name, obj in list(module_globals.items()):
        # ``@method`` handlers conventionally use ``def _``.  The registry installs those
        # under their RPC names, so publishing the placeholder would only make separately
        # extracted handler modules overwrite each other during server import.
        if (name == "_" or name.startswith("__") or name in _PLUMBING or name in skip
