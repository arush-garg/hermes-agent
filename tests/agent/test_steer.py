...
def _bare_agent() -> AIAgent:
    ...
    agent._pending_yolo_action = None
...
class TestYoloSteer:
    def test_queued_toggle_applies_after_a_tool_batch_boundary(self):
        from tools.approval import disable_session_yolo, is_session_yolo_enabled

        agent = _bare_agent()
        session_key = "test-yolo-steer-boundary"
        disable_session_yolo(session_key)
        agent.steer_yolo(session_key, True)

        agent._apply_pending_yolo_action()

        assert is_session_yolo_enabled(session_key) is True
        assert agent._pending_yolo_action is None
        disable_session_yolo(session_key)
