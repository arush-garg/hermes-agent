...
class ClawHubSource(GuardedFetchMixin, SkillSource):
    ...
    @staticmethod
    def _get_json_retry(url: str, *, timeout: int = 30, params: Optional[Dict[str, Any]] = None,
                        attempts: int = 4) -> Optional[Any]:
        """GET+JSON decode with exponential backoff on transient failures.

        The ClawHub API occasionally flakes mid-crawl (429/502/503/504); retrying lets a full
        catalog walk complete instead of shipping a degenerate index. Non-retryable statuses,
        and transport/decode errors once attempts are exhausted, return None.
        """
        for page_attempt in range(attempts):
            try:
                resp = httpx.get(url, params=params, timeout=timeout)
            except httpx.HTTPError:
                if page_attempt < attempts - 1:
    ...
    def _load_catalog_index(self, max_items: int = 0) -> List[SkillMeta]:
        ...
        for _ in range(750):
            ...
            data = self._get_json_retry(f"{self.BASE_URL}/skills", timeout=30, params=params)
