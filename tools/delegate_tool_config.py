...
def _delegation_uses_parent_route(
    ...
    """Whether the delegation resolves to the parent's own provider endpoint.

    A delegation pin that merely restates the parent's route must keep inheriting the parent's
    fallback chain; a pin that changes provider or endpoint must not (a quiet-mode child that
    silently reroutes after auth/429 is the silent-drag #80450 guards against).
    """
    from agent.backend_identity import BackendIdentity, same_endpoint

    parent_provider = getattr(parent_agent, "provider", None)
    parent_base_url = _inherit_parent_base_url(parent_agent, getattr(parent_agent, "base_url", None))
    parent_route = BackendIdentity.build(provider=parent_provider, base_url=parent_base_url)
    if override_base_url and not parent_base_url:
        # Parent uses the provider default while the child names an endpoint; equivalence is
        # unprovable, so preserve #80450's fail-loud posture.
        return False
    child_route = BackendIdentity.build(
        provider=override_provider or parent_provider, base_url=override_base_url or parent_base_url,
    )
    return parent_route.provider == child_route.provider and same_endpoint(parent_route, child_route)
...
def _resolve_child_runtime(
    ...
    kwargs: Dict[str, Any] = {
        ...
        "fallback_model": _resolve_child_fallback_chain(
            ...
            # A pin that restates the parent's route is not a route change: the parent's chain
            # stays valid, so keep inheriting it.
            pinned=not _delegation_uses_parent_route(parent_agent, override_provider, override_base_url)),
