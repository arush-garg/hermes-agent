...
def _translate_claude_mcp_entry(entry: dict) -> Optional[dict]:
    ...
    Project-local ``.mcp.json`` files use the ``mcpServers`` shape popularized by Claude
    Code/Cursor. Hermes already accepts the same transport keys for stdio (``command``/``args``/
    ``env``) and remote transports (``url``), but Claude-style entries commonly include a ``type``
    discriminator. Strip that discriminator after validating the entry and map ``type: sse`` to
    Hermes' explicit ``transport: sse`` flag.
    """
...
def _load_project_mcp_json(workspace_root: Optional[str] = None) -> Dict[str, dict]:
    ...
    The file follows Claude/Cursor's ``{"mcpServers": {...}}`` shape. Invalid files or entries are
    ignored so a bad project config cannot disable global MCP servers.
    """
...
def _load_mcp_config() -> Dict[str, dict]:
    """``mcp_servers`` from config.yaml plus project ``.mcp.json`` as ``{name: config}`` (empty on
    error / safe mode), ``${VAR}`` interpolated. Project-local servers are additive-only."""
    try:
        ...
        try:
            # Project-local .mcp.json is additive-only: global/plugin names win on conflict, so a
            # project file can only ADD MCP servers.
            project_servers = _load_project_mcp_json()
            for name, cfg in _filter_suspicious_mcp_servers(project_servers).items():
                if name in safe_servers:
                    logger.warning("Project MCP server '%s' conflicts with existing config; skipping", name)
                    continue
                interpolated = _interpolate_env_vars(cfg)
                if isinstance(interpolated, dict):
                    _warn_hidden_whitespace(name, interpolated)
