...
_BROWSER_NAVIGATE_DESCRIPTION = (
    ...
    "For simple information retrieval, prefer a lightweight retrieval tool when one is available (faster, cheaper). "
    "For plain-text endpoints — URLs ending in .md, .txt, .json, .yaml, .yml, .csv, .xml, raw.githubusercontent.com, or any documented API endpoint — prefer an available text-extraction or terminal-fetch tool; the browser stack is overkill and much slower for these. "
...
# Drop the retrieval-tool hint entirely when no web tool is registered, so the model is never
# pointed at a tool that is not in its toolset.
...
def _browser_navigate_schema_overrides() -> dict:
    """Hide retrieval-tool cross references when no web tools are available."""
    try:
...
# Per-tool dynamic schema overrides, applied by the registration loop below.
_BROWSER_SCHEMA_OVERRIDES = {"browser_navigate": _browser_navigate_schema_overrides}
...
for _name, _emoji, _check_fn, _defaults, *_extra in _BROWSER_TOOL_TABLE:
    ...
    registry.register(name=_name, toolset="browser", schema=_BROWSER_SCHEMA_MAP[_name],
                      ...
                      dynamic_schema_overrides=_BROWSER_SCHEMA_OVERRIDES.get(_name))
