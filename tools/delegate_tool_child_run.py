...
class _Heartbeat:
    ...
    def __init__(self, child: Any, parent_agent: Any, task_index: int):
        ...
        # Inactivity clock: last time the child made API progress (new call, tool change, or a
        # running tool). Reset continuously inside a tool so long builds are not interrupted;
        # accumulates only while the child is idle between API calls.
        self.last_progress_ts: float = time.monotonic()
    ...
    def tick(self):
        ...
        try:
            ...
            if child_iter > last_seen["iter"] or child_tool != last_seen["tool"] or activity_advanced:
                ...
                # Actively inside a long-running tool — reset the inactivity clock so legitimately
                # slow tools (builds, downloads) are never interrupted while still running.
                last_seen["stale"] += 1
                self.last_progress_ts = time.monotonic()
            else:
                ...
                idle_secs = time.monotonic() - self.last_progress_ts
                if child_timeout is not None and idle_secs >= child_timeout:
                    logger.warning(
                        "Subagent %d inactivity timeout: no API call for %.0fs (tool=<none>) — interrupting",
                        task_index, idle_secs,
                    )
                    with _quiet("Subagent inactivity interrupt failed"):
...
class _ChildRun:
    ...
    heartbeat: Any = None
    # When true the child's first turn is seeded with a deterministic pruned copy of the parent's
    # persisted history (see ``_compressed_parent_history``).
    ...
    def await_child(self) -> tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]], bool]:
        ...
        # give_compressed_context: seed the child's first turn with a deterministic pruned copy of
        # the parent's persisted history (None otherwise, so the child starts fresh).
        if self.give_compressed_context:
            from tools.delegate_tool import _compressed_parent_history
            conversation_history = _compressed_parent_history(self.parent_agent)
        else:
        ...
        if not is_timeout:
            ...
            _err = (
                f"Subagent hit failsafe timeout ({child_timeout}s) without making any API call — the child never "
                f"reached its first LLM request and did not exit after inactivity interrupt (prompt construction, "
                f"credential resolution, or transport may be stuck)."
            )
        else:
            _err = (
