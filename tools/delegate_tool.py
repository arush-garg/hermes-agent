...
Spawns child AIAgent instances with a fresh conversation by default — or an optional
deterministic-pruned copy of the parent's history (``give_compressed_context``) — their own
task_id (terminal session, file-ops cache), the parent's toolsets minus child-blocked
...
# job_id -> {status, n_tasks, completed, results, started_at, done_event}. Populated by async
# delegate_task calls (async_mode=True) and consumed by get_delegate_result.
...
def _compressed_parent_history(parent_agent) -> Optional[List[Dict[str, Any]]]:
    ...
    Feeds ``give_compressed_context``. The shared helper snapshots nested payloads before
    pruning, so parent progress cannot mutate this child's handoff while it is assembled.
    """
...
def _run_single_child(
    ...
    owner_transport: Any = None, owner_session_record: Any = None, give_compressed_context: bool = False,
    **_kwargs,
) -> Dict[str, Any]:
    ...
    * ``"completed"``       — normal finish. See #97655.
    ...
    run = _ChildRun(
        ...
        give_compressed_context=give_compressed_context,
    )
            ...
            "tasks": {
                ...
                "items": {
                    ...
                    "properties": {
                        "goal": _p(
                            "string",
                            "What this subagent should accomplish. Be specific and self-contained. Unless "
                            "give_compressed_context is true, it knows nothing about your conversation history.",
                        ),
                        "context": _p(
                            "string",
                        ...
                        "give_compressed_context": _p(
                            "boolean",
                            "Optional, default false. When true, give this child a deterministic pruned copy of "
                            "prior conversation history. Large historical tool outputs are summarized without "
                            "another model call. Leave false for a fresh isolated child.",
                        ),
                        "images": _p(
                            "array",
...
def get_delegate_result(
    ...
    When ``wait=False`` returns immediately with the current status.
    """
    ...
    if job["status"] == "done":
        ...
        return json.dumps({"results": results, "total_duration_seconds": duration}, ensure_ascii=False)
    ...
    return tool_error(f"Job '{job_id}' timed out after {timeout}s waiting for children to finish.")


GET_DELEGATE_RESULT_SCHEMA = {
    "name": "get_delegate_result",
    "description": (
        "Collect the result of an async delegate_task call. Pass the job_id returned by "
        "delegate_task(async_mode=True). By default blocks until all children finish (wait=true). "
        "Set wait=false to poll without blocking — returns status='running' with completed/total "
        "counts when children are still working."
    ),
    "parameters": {
        "type": "object",
        "properties": {
