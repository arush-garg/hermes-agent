...
# Per-profile ContextVar isolation for TERMINAL_* configuration. The gateway's
# ``_profile_runtime_scope`` installs a routed profile's terminal env/config into these vars so
# concurrent turns for different profiles do not leak backend/container mounts/secrets via the
# process-global os.environ (#68559 / #99986). The ContextVars themselves live in
# ``tools.terminal_tool_config`` so ``_tenv`` — the single scope-aware reader — can honour them;
# they are re-exported here because callers and tests import them from this module.


def _parse_terminal_config_from_getter(env_get) -> Dict[str, Any]:
    """Build the terminal config dict reading every ``TERMINAL_*`` key through *env_get*.

    Mirrors :func:`_get_env_config`, but never consults the ambient environment, so a routed
    profile's settings can be parsed without touching process-global ``os.environ``.
    """
    ...
    if env_type == "docker" and mount_docker_cwd:
        candidate = os.path.abspath(os.path.expanduser(env_get("TERMINAL_CWD", "") or _safe_getcwd()))
        if _is_host_cwd(candidate) or (
        ...
        logger.info("Ignoring TERMINAL_CWD=%r for %s backend (host/relative path won't work in sandbox). "
                    "Using %r instead.", cwd, env_type, default_cwd)
        cwd = default_cwd
    return {
        "env_type": env_type,
...
def build_isolated_terminal_config(isolated_env: Dict[str, str]) -> Dict[str, Any]:
    """Parse a profile's ``TERMINAL_*`` env mapping into a terminal config dict."""
...
from tools.terminal_tool_config import (
    _TERMINAL_CONFIG_ISOLATION, _TERMINAL_ENV_ISOLATION, _is_container_backend, _is_host_cwd,
    _is_unusable_container_cwd, _parse_env_var, _plugin_env_flag, _quiet, _safe_getcwd, _tenv, _tenv_bool,
    _term_env_get, get_terminal_config_isolation, reset_terminal_config_isolation,
    reset_terminal_env_isolation, set_terminal_config_isolation, set_terminal_env_isolation,
)
...
def _ensure_terminal_env_bridged() -> None:
    ...
    if get_terminal_scope() is not None:
        return
    # An installed per-profile isolation layer owns TERMINAL_* for this context; bridging the
    # launch profile's config into the process env would defeat it (#68559 / #99986).
...
def _get_env_config() -> Dict[str, Any]:
    ...
    # Isolated runtime fast path: a per-profile worker already built the parsed config and
    # installed it via ContextVar. Returning it avoids reparsing env vars on every tool call
    # inside a multiplexed turn and guarantees the routed profile's backend is used rather
    # than the gateway's process-global TERMINAL_ENV (#68559 / #99986).
    iso_cfg = _TERMINAL_CONFIG_ISOLATION.get()
    if iso_cfg is not None:
        return iso_cfg
    # Env-isolation without a pre-parsed config (fallback): parse the installed env dict
