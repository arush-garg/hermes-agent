...
# The gateway's ``_profile_runtime_scope`` installs a routed profile's terminal env (and the
# config parsed from it) into these ContextVars so concurrent turns for different profiles do
# not leak a backend/container mount/secret through process-global ``os.environ`` (#68559 /
# #99986). They live here — next to ``_tenv`` — so the single scope-aware reader honours them;
# ``tools.terminal_tool`` re-exports the public names for callers and tests.
from contextvars import ContextVar, Token  # noqa: E402

_TERMINAL_ENV_ISOLATION: ContextVar = ContextVar("_TERMINAL_ENV_ISOLATION", default=None)
_TERMINAL_CONFIG_ISOLATION: ContextVar = ContextVar("_TERMINAL_CONFIG_ISOLATION", default=None)


def set_terminal_env_isolation(env_dict: Optional[dict]) -> Token:
    return _TERMINAL_ENV_ISOLATION.set(env_dict)


def reset_terminal_env_isolation(token: Token) -> None:
    _TERMINAL_ENV_ISOLATION.reset(token)


def set_terminal_config_isolation(config: Optional[dict]) -> Token:
    return _TERMINAL_CONFIG_ISOLATION.set(config)


def reset_terminal_config_isolation(token: Token) -> None:
    _TERMINAL_CONFIG_ISOLATION.reset(token)


def get_terminal_config_isolation() -> Optional[dict]:
    return _TERMINAL_CONFIG_ISOLATION.get()


def _term_env_get(name: str, default: str = "") -> str:
    """Isolation-first read of a ``TERMINAL_*`` variable (falls back to ``os.getenv``)."""
    if name.startswith("TERMINAL_"):
        iso = _TERMINAL_ENV_ISOLATION.get()
        if iso is not None:
            return iso.get(name, default)
...
def _tenv(name: str, default: str = "") -> str:
    ...
    An installed isolation ContextVar (the gateway's per-profile routing layer) wins over the
    ambient ``terminal_scope``, because it is installed for exactly this turn."""
    if name.startswith("TERMINAL_"):
        iso = _TERMINAL_ENV_ISOLATION.get()
        if iso is not None:
            return iso.get(name, default)
