...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        script = (
            ...
            "fi; "
            'cat > "$tmp"; '
            # Ensure durable write before atomic rename — fsync temp file (#99869 proposal 1:
            # write to <file>.tmp -> fsync -> rename). Best-effort: sync available on GNU
            # coreutils/macOS/busybox; fall back to global sync or a no-op where sync is absent.
            ...
            'mv -f "$tmp" "$t"; '
            # Best-effort fsync of the target after rename so a power loss immediately after mv
            # doesn't lose the new inode.
