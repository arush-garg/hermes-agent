...
def _get_exclude_mcp_toolsets() -> set:
    """MCP toolset names ``delegation.exclude_mcp_toolsets`` keeps out of every child."""
    from tools.delegate_tool import _load_config
    exclude = _load_config().get("delegation", {}).get("exclude_mcp_toolsets") or []
...
def _resolve_child_toolsets(
    ...
    if toolsets:
        ...
        if _get_inherit_mcp_toolsets():
            # Append any parent MCP toolsets missing from the narrowed child, minus the ones
            # ``delegation.exclude_mcp_toolsets`` opts out of (e.g. trading tools a child must
            # never reach).
            excluded = _get_exclude_mcp_toolsets()
            child_toolsets += [
                name for name in sorted(parent_toolsets)
                if _is_mcp_toolset_name(name) and name not in child_toolsets and name not in excluded
            ]
