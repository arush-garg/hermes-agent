...
def _gateway_provider_error_reply(text: str) -> str:
    ...
    categories = ("authentication", "policy", "rate_limit", "connection")
    ...
    for name, (pattern, reply) in zip(categories, _PROVIDER_ERROR_REPLIES):
        if pattern.search(text):
            category, default = name, reply
            break
    try:
        configured = _load_gateway_config().get("gateway", {}).get(
...
def _profile_runtime_scope(
    ...
    """Bind a routed profile's home, secrets, and terminal policy for one activity."""
    from contextlib import ExitStack
    from hermes_constants import set_hermes_home_override, reset_hermes_home_override
    from agent.secret_scope import set_secret_scope, reset_secret_scope

    def _reset_best_effort(reset, token):
        try:
            reset(token)
        except Exception:
            logger.debug("Profile scope cleanup failed", exc_info=True)

    with ExitStack() as cleanup:
        home_token = set_hermes_home_override(str(profile_home))
        cleanup.callback(_reset_best_effort, reset_hermes_home_override, home_token)
        ...
        cleanup.callback(_reset_best_effort, reset_secret_scope, secret_token)

        # A missing home is tolerated for synthetic profile reads in tests. A real
        # routed profile must establish all terminal authority before entering a turn.
        if Path(profile_home).exists():
            try:
                from hermes_cli.config import apply_terminal_config_to_env
                from tools.terminal_scope import (
                    build_profile_terminal_scope, set_terminal_scope, reset_terminal_scope,
                )
                from tools.terminal_tool import (
                    set_terminal_env_isolation, reset_terminal_env_isolation,
                    set_terminal_config_isolation, reset_terminal_config_isolation,
                    _parse_terminal_config_from_getter, _term_env_get,
                )

                isolated_env = build_profile_terminal_scope(Path(profile_home))
                apply_terminal_config_to_env(env=isolated_env, override=True)
                env_token = set_terminal_env_isolation(isolated_env)
                cleanup.callback(_reset_best_effort, reset_terminal_env_isolation, env_token)
                isolated_cfg = _parse_terminal_config_from_getter(_term_env_get)
                cfg_token = set_terminal_config_isolation(isolated_cfg)
                cleanup.callback(_reset_best_effort, reset_terminal_config_isolation, cfg_token)
                scope_token = set_terminal_scope(isolated_env)
                cleanup.callback(_reset_best_effort, reset_terminal_scope, scope_token)
            except Exception as exc:
                raise RuntimeError(
                    f"terminal isolation admission failed for {profile_home}: {exc}"
                ) from exc
        yield
...
class GatewayRunner(
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        # Routed profiles use isolated turn workers when tenant isolation is enabled.
