...
class GatewayTurnMixin:
    ...
    async def _hmwa_resolve_session(self, event, source):
        ...
        # Discord /yolo can arrive in the parent channel while the agent's
        # auto-threaded reply uses a fresh session key. Inherit only on first
        # dispatch so an explicit disable inside the thread remains effective.
        if getattr(source, "parent_chat_id", None) and getattr(source, "chat_type", None) == "thread":
            known_sources = getattr(self, "_session_sources", None)
            if not (known_sources and session_key in known_sources):
                try:
                    ...
                    if not is_session_yolo_enabled(session_key):
                        parent_source = dataclasses.replace(
                            source, chat_id=source.parent_chat_id, chat_type="group",
                            thread_id=None, parent_chat_id=None, prospective_thread_id=None,
                        )
                        if is_session_yolo_enabled(self._session_key_for_source(parent_source)):
                            enable_session_yolo(session_key)
                except Exception:
                    logger.debug("Parent channel YOLO inheritance failed", exc_info=True)
    ...
    async def _run_agent(
        ...
        if (
            ...
            and getattr(supervisor.config, "mode", "off") not in ("off", "in_process")
        ):
            ...
            correlation_id = (
                f"{session_key}:{run_generation}:{session_id}"
                if session_key else f"{session_id}:{run_generation}"
            )
            result = await supervisor.run_turn(
                profile=validated or profile, source=source, message=message,
                correlation_id=correlation_id, gateway=self, session_id=session_id,
                session_key=session_key, context_prompt=context_prompt, history=history,
                **{key: value for key, value in turn_kwargs.items() if key != "session_key"},
            )
            if not self._validate_tenant_delivery(
                result, expected_profile=validated or profile,
                expected_correlation=correlation_id, expected_session_id=session_id,
