...
class CLIInfoMixin:
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        from hermes_cli.bang_shell import (
            bang_shell_enabled, check_bang_approval, is_bang_command,
                ...
                async def _run_interactive():
                    if command:
                        ...
                        if not approval.get("approved"):
                            ...
                            return
                    exit_code = await run_bang_command_interactive(app, command or None, cwd=cwd)
                    if exit_code:
        ...
        if not command:  # bare `!` — show what the feature does
            self._console_print("[dim]Type !<command> to run a local shell command.[/]")
            return True
