...
def _user_provider_live_cache_has_model(st: _Switch) -> bool:
    """True when the cached live model listing for this user-configured ``providers:`` slug lists
    the requested model (and the cache is fresh). Fail-open: any cache error means "no opinion"."""
    if not (st.user_providers and st.target_provider in st.user_providers):
        return False
    try:
        import time as _time_mod

        from hermes_cli.models import _PROVIDER_MODELS_CACHE_TTL, _load_provider_models_cache

        entry = _load_provider_models_cache().get(f"user:{st.target_provider}")
        return bool(
            isinstance(entry, dict)
            and isinstance(entry.get("models"), list)
            and entry["models"]
            and (_time_mod.time() - float(entry.get("at", 0))) < _PROVIDER_MODELS_CACHE_TTL
            and st.new_model in entry["models"]
        )
    except Exception:
        return False
...
def _validate_switch(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if not validation.get("accepted"):
        if _user_provider_live_cache_has_model(st):
            # validate_requested_model probes with api_key="no-key-required" (the sentinel a
            # keyless endpoint gets), so a keyless ``providers:`` entry (e.g. omniroute) can
            # report 0 models and reject a perfectly valid selection. The picker's live disk
            # cache is the user's own endpoint reporting its catalog — trust it.
            validation = {"accepted": True, "persist": True, "recognized": False,
