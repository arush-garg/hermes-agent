...
class CLITuiMixin:
    ...
    def _tui_handle_voice_record(self, event):
        ...
        if not self._voice_tts_done.is_set():
            try:
                ...
                pass
        # The pending responses belong to the earlier turn and must not play over this recording.
        try:
            while True:
            ...
            pass
        if not getattr(self, "_voice_push_to_talk_only", False):
            with self._voice_lock:
    ...
    def _tui_enter_inline_command(self, event, text: str, has_images: bool) -> bool:
        ...
        if text.startswith("!") and self.handle_bang_shell(text):
            pass  # interactive shell gets prompt_toolkit's terminal on this UI thread
            ...
            or self._busy_inline_command(text, has_images, ("yolo", "goal"))):
    ...
    def _tui_init_run_state(self):
        ...
        self._voice_tts_queue = queue.Queue()
