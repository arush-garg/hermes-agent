...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ...
    # Claude Code CLI / subscription: external subprocess (agent.claude_cli_client.ClaudeCLIClient),
    # routed over chat_completions. Registered here so ``resolve_provider()`` knows it; the runtime
    # ladder resolves the binary via the provider profile's process_command.
    ProviderConfig(
        "claude-cli", "Claude Code (CLI / subscription)", "external_process",
