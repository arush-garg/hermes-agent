...
class CLICommandsMixin:
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            # No explicit argument → show interactive curses picker (fork feature).
            sessions = self._list_recent_sessions(limit=20)
            if not sessions:
                _cp("  No previous sessions found.")
                _cp("  Tip:   Use /history or `hermes sessions list` to find sessions.")
                return

            # A captured or redirected terminal cannot host curses. Keep the numbered list
    ...
    def _handle_background_command(self, cmd: str):
        ...
        def produce():
            ...
            try:
                ...
                # Inherit the parent session's YOLO bypass (fork feature). /yolo flags only
                # THIS session's id in tools.approval._session_yolo; a background task runs as
                # its own session (task_id), so without this transfer every dangerous-command
                # check in the task re-prompts even though the user toggled YOLO on.
