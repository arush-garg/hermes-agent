...
class CLIStreamMixin:
    ...
    def _on_thinking(self, text: str) -> None:
        ...
        def show():
            ...
            if not text:
                ...
                invoke_hook("on_spinner_end", text="")
            else:
                invoke_hook("on_spinner_start", text=text, source="thinking")
    ...
    def _stream_delta(self, text) -> None:
        ...
        if text is None:
            self._flush_stream()
            spoken = getattr(self, "_stream_tts_buf", "").strip()
            if (spoken and getattr(self, "_voice_tts", False)
                    and not getattr(self, "_elevenlabs_streaming_active", False)):
                transition = "and" if not self._voice_tts_queue.empty() else ""
                self._voice_tts_enqueue(spoken, transition_word=transition)
            self._reset_stream_state()
            return
        if not text:
            return
        if (getattr(self, "_voice_tts", False)
                and not getattr(self, "_elevenlabs_streaming_active", False)):
    ...
    def _on_tool_progress(self, event_type: str, function_name: str = None, preview: str = None, function_args: dict = None, **kwargs):
        ...
        if event_type.startswith("subagent."):
            context = {**kwargs, "goal": function_name, "text": preview}
            ...
            try:
                ...
                hermes_logging.log_event("subagent", event_type, **context)
            except Exception:
                pass
            if event_type in {"subagent.spawn_requested", "subagent.start", "subagent.complete"}:
                status = kwargs.get("status", "completed")
                label = ("spawning subagent" if event_type.endswith("spawn_requested") else
                         "subagent started" if event_type.endswith("start") else
                         f"subagent {status}")
                self._spinner_text = f"⛓ {label}: {function_name or 'unknown task'}"
                self._invalidate()
            return
        ...
        if function_name and not function_name.startswith("_"):
            ...
            from hermes_cli.plugins import invoke_hook
            invoke_hook("on_spinner_start", text=self._spinner_text, source="tool")
